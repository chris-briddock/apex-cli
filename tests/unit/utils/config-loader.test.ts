...
import {
  ...
  validateColors,
  validateFeatures
} from '../../../cli/utils/config-loader.ts';
...
describe('config-loader', () => {
  ...
  describe('defaultConfig', () => {
    ...
    it('should have core features enabled by default', () => {
      assert.strictEqual((defaultConfig.features as Record<string, unknown>).display, true);
      assert.strictEqual((defaultConfig.features as Record<string, unknown>).flexbox, true);
      assert.strictEqual((defaultConfig.features as Record<string, unknown>).grid, true);
      assert.strictEqual((defaultConfig.features as Record<string, unknown>).typography, true);
      assert.strictEqual((defaultConfig.features as Record<string, unknown>).colors, true);
    });

    it('should have standard breakpoints', () => {
      const breakpoints = defaultConfig.breakpoints as Record<string, unknown>;
      assert.strictEqual(breakpoints.sm, '320px');
      assert.strictEqual(breakpoints.md, '768px');
      assert.strictEqual(breakpoints.lg, '1024px');
      assert.strictEqual(breakpoints.xl, '1280px');
    });

    it('should have spacing scale', () => {
      const spacing = defaultConfig.spacing as Record<string, unknown>;
      assert.strictEqual(spacing['1'], '0.25rem');
      assert.strictEqual(spacing['4'], '1rem');
      assert.strictEqual(spacing['8'], '2rem');
    });

    it('should have primary color configuration', () => {
      const colors = defaultConfig.colors as Record<string, unknown>;
      assert.ok(colors.primary);
      const primary = colors.primary as Record<string, unknown>;
      assert.strictEqual(typeof primary.hue, 'number');
      assert.strictEqual(typeof primary.chroma, 'number');
      assert.ok(primary.lightnessScale);
    });
  });
  ...
  describe('loadConfig', () => {
    it('should return default config when file does not exist', () => {
      ...
      assert.strictEqual((config.features as Record<string, unknown>).display, true);
    });
    ...
    it('should load JSON config file', () => {
      ...
      assert.strictEqual((config.features as Record<string, unknown>).display, false);
      assert.strictEqual((config.features as Record<string, unknown>).flexbox, true);
      // Other features should still be from default
      assert.strictEqual((config.features as Record<string, unknown>).grid, true);
    });
    ...
    it('should merge user config with defaults', () => {
      ...
      const config = loadConfig('test.config.json');
      assert.strictEqual((config.breakpoints as Record<string, unknown>).sm, '400px');
      assert.strictEqual((config.breakpoints as Record<string, unknown>).md, '768px'); // from default
    });
    ...
    it('should throw on invalid config type', () => {
      ...
      try {
        ...
        assert.ok((error as Error).message.includes('Failed to load config'));
      }
    });
    ...
    it('should validate feature toggles are booleans', () => {
      ...
      try {
        ...
        assert.ok((error as Error).message.includes('validation failed'));
      }
    });
  ...
  describe('validation functions', () => {
    ...
    it('mergeConfig should deep merge nested objects', () => {
      ...
      const result = mergeConfig(target, source);
      assert.strictEqual((result.features as Record<string, unknown>).display, false);
      assert.strictEqual((result.features as Record<string, unknown>).flexbox, true);
    });
    ...
    it('mergeConfig should not mutate original target', () => {
      ...
      const result = mergeConfig(target, source);
      assert.strictEqual((target.nested as Record<string, unknown>).c, undefined);
      assert.strictEqual((result.nested as Record<string, unknown>).c, 3);
    });
    ...
    it('mergeConfig should add new nested properties', () => {
      ...
      const result = mergeConfig(target, source);
      assert.strictEqual((result.features as Record<string, unknown>).display, true);
      assert.strictEqual((result.features as Record<string, unknown>).flexbox, true);
      assert.strictEqual((result.features as Record<string, unknown>).grid, true);
    });
