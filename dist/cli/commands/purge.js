...
import { configExists, createBackup, formatDiff, generateDiff, generateSummary, updateConfigFile, validateChanges } from '../utils/config-modifier.js';
...
export async function purgeCommand(options) {
    ...
    let scanResult;
    try {
        scanResult = await scanDirectories(srcDirs.map(d => resolve(cwd, d)), { onProgress: createProgressCallback(options.verbose) });
    }
    catch (error) {
