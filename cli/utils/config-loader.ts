...
export function loadConfig(configPath: string): Record<string, unknown> {
  ...
  try {
    ...
    if (configPath.endsWith('.json')) {
      ...
      const loadedModule = require(fullPath) as { default?: Record<string, unknown> } | Record<string, unknown>;
      userConfig = (loadedModule as { default?: Record<string, unknown> }).default || loadedModule;
    }
  ...
  for (const key in source) {
    if (Object.hasOwn(source, key)) {
      if (isObject(source[key]) && isObject(target[key])) {
        output[key] = mergeConfig(target[key] as Record<string, unknown>, source[key] as Record<string, unknown>);
      } else {
 ...
 */
export function validateFeatures(features: Record<string, unknown>, errors: string[]): void {
 ...
 */
export function validateBreakpoints(breakpoints: Record<string, unknown>, errors: string[]): void {
 ...
 */
export function validateColors(colors: Record<string, unknown>, errors: string[]): void {
  for (const [colorName, colorConfig] of Object.entries(colors)) {
    validateColorHue(colorConfig as Record<string, unknown>, colorName, errors);
    validateColorChroma(colorConfig as Record<string, unknown>, colorName, errors);
  }
}
 ...
 */
export function validateColorHue(colorConfig: Record<string, unknown>, colorName: string, errors: string[]): void {
 ...
 */
export function validateColorChroma(colorConfig: Record<string, unknown>, colorName: string, errors: string[]): void {
...
export function validateConfig(config: Record<string, unknown>): ValidationResult {
  ...
  if (config.features) {
    validateFeatures(config.features as Record<string, unknown>, errors);
  }

  if (config.breakpoints) {
    validateBreakpoints(config.breakpoints as Record<string, unknown>, errors);
  }

  if (config.colors) {
    validateColors(config.colors as Record<string, unknown>, errors);
  }
