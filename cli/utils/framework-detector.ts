...
export const FRAMEWORKS: Record<string, FrameworkDefinition> = {
  next: {
    name: 'Next.js',
    detect: (pkg: PackageJson) => !!(pkg.dependencies?.next || pkg.devDependencies?.next),
  ...
  nuxt: {
    name: 'Nuxt',
    detect: (pkg: PackageJson) => !!(pkg.dependencies?.nuxt || pkg.devDependencies?.nuxt),
    entryFiles: ['nuxt.config.js', 'nuxt.config.js'],
    cssConfig: "css: ['~/apexcss/apex.css']",
    importStatement: null // Nuxt uses CSS config in nuxt.config
  },
  react: {
    name: 'React',
    detect: (pkg: PackageJson) => !!(pkg.dependencies?.react && !pkg.dependencies?.next),
  ...
  vue: {
    name: 'Vue',
    detect: (pkg: PackageJson) => !!(pkg.dependencies?.vue && !pkg.dependencies?.nuxt),
  ...
  angular: {
    name: 'Angular',
    detect: (pkg: PackageJson) => !!pkg.dependencies?.['@angular/core'],
  ...
  svelte: {
    name: 'Svelte',
    detect: (pkg: PackageJson) => !!(pkg.dependencies?.svelte || pkg.devDependencies?.svelte),
  ...
  astro: {
    name: 'Astro',
    detect: (pkg: PackageJson) => !!(pkg.dependencies?.astro || pkg.devDependencies?.astro),
...
export function detectFramework(cwd = process.cwd()): FrameworkInfo {
  ...
  let pkg: PackageJson;
  try {
    pkg = JSON.parse(readFileSync(packageJsonPath, 'utf-8')) as PackageJson;
  } catch {
    return {
      id: 'vanilla',
      ...FRAMEWORKS.vanilla,
 ...
 */
export function getAvailableFrameworks(): Array<{ id: string; name: string }> {
  return [
 ...
 */
export function getRecommendedOutputDir(_frameworkId?: string): string {
 ...
 */
export function getFrameworkConfigApproach(frameworkId: string): ConfigApproach {
  const approaches: Record<string, ConfigApproach> = {
